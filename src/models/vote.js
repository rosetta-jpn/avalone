...
function Vote(selector,group_sz,voter_sz){
    this.selector = selector;
    this.group = [];// User Array
    this.group_sz = group_sz;
    this.voter_map = {};//Voter map User-> Yes/No(true/false)
    this.voter_sz = voter_sz;
    this.vote_count = 1;
    this.state = States[0];
}

Vote.prototype.add_group = function(pusher,add_man) {
...
Vote.prototype.remove_group = function(remover,remove_man){
...
Vote.prototype.is_vote_success = function(){
    var agree = 0;
    var disagree = 0;
...
Vote.prototype.change_voter_map(voter,vote_res){
    
    voter_map[voter.id] = vote_res;

}

Vote.prototype.go_vote(){
    if(this.state === States[0] && this.group.length == this.group_sz){
        this.state = States[1];
    }
}

Vote.prototype.judge(){
